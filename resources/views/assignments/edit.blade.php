    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8">
                    <h1 class="text-2xl font-bold mb-8 text-center">Edit: {{ $assignment->title }}</h1>

                    <form action="{{ route('assignments.update', $assignment->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        
                        <input type="text" name="title" value="{{ $assignment->title }}" placeholder="Title" 
                               class="w-full bg-gray-200 px-4 py-3 mb-4 rounded border-0" required>

                        <textarea name="description" placeholder="Description" rows="4"
                                  class="w-full bg-gray-200 px-4 py-3 mb-4 rounded border-0" required>{{ $assignment->description }}</textarea>
                        ...
                        <div class="flex gap-2 mb-8">
                            ...
                            <span id="file-name" class="bg-gray-100 px-4 py-2 flex-1 text-sm text-gray-500">
                                {{ $assignment->file_path ? 'Current file exists' : 'No File Selected' }}
                            </span>
                        </div>
